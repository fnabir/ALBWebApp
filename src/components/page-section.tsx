    ...
    <section id={sectionId} aria-labelledby={`${sectionId}-section`} className={`flex items-center justify-center ${className}`}>
      <div className="container px-6 py-12 md:py-20">
        <div className="flex flex-col items-center text-center max-w-4xl mx-auto">
          ...
          <h1 className={`text-3xl md:text-4xl lg:text-5xl font-bold tracking-tighter mb-3 lg:mb-6 ${headingClass}`}>{heading}</h1>
          {subheading && <p className={`text-muted-foreground md:text-2xl lg:text-lg mb-8 lg:mb-12 ${subheadingClass}`}>{subheading}</p>}
        </div>
        {children}
      </div>
    </section>
