...
const FormSchema = z.object({
  ...
  contact: z
    .string()
    .nonempty('Contact info is required')
    .min(11, 'Must be at least 11-digit mobile number or email.'),
...
export default function GetQuote() {
  ...
  const [personOptions, setPersonOptions] =
    useState<OptionsInterface[]>(PassengerLoadOptions);
  useEffect(() => {
  ...
  async function onSubmit(data: FormData) {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(
          'Failed to verify reCAPTCHA. Refresh the page and try again.',
        );
      }
    } catch {
      ...
      render={({ field }) => (
        <FormItem className="grid-cols-4 gap-y-1">
          <FormLabel className="col-span-1">
            {label}
            {required && (
              <span className="text-red-500 -ml-1.75 -translate-y-1">*</span>
            )}
          </FormLabel>
